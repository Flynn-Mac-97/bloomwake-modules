...
namespace Flynn.Resources
{
    ...
    public class ResourceNode : MonoBehaviour
    {
        ...
        /// <summary>Health remaining, and the same value as 0..1. Read-only: the node stays the
        /// only thing that can change it. Added so presentation (a blight aura, a decrementing
        /// overlay count) can reflect damage without owning a second copy of the number.</summary>
        ...
        public void Hit(int damage)
        {
            ...
            if (_config.echoShardItem != null && Random.value < _config.echoShardChancePerHit)
            {
                ...
                // land it on the ground; the drop hop, the rest and the pickup flight are the
                // item's own business now (WorldItemFeel), not a straight flight at the player
                WorldItemSpawner.Spawn(_config.echoShardItem, 1, pos);
        ...
        private void SpawnDrops()
        {
            ...
            foreach (var drop in _config.drops)
            {
                ...
                // One object PER unit, not one stack of N: each lands on the ground and lives its
                // own drop / rest / pickup (WorldItemFeel), which is what makes a felled tree read
                // as a scatter of logs instead of a single pile that vanishes in one go.
                for (int i = 0; i < count; i++)
                    WorldItemSpawner.Spawn(drop.item, 1, pos);
