...
namespace Flynn.Events
{
    ...
    public readonly struct ToolSwingStarted
    {
        ...
        public readonly float SweepDuration;
        public readonly float SweepArcDegrees;
        public readonly float SweepRadius;
        public ToolSwingStarted(ToolType toolType, Vector3 aimPoint, float sweepDuration, float sweepArcDegrees, float sweepRadius)
        { ToolType = toolType; AimPoint = aimPoint; SweepDuration = sweepDuration; SweepArcDegrees = sweepArcDegrees; SweepRadius = sweepRadius; }
    }
    ...
    // ── Scan events live in Flynn.Runtime ────────────────────────────────────
    // They carry a ScanTarget, and ScanTarget needs PlayerInventory, which sits above this
    // layer. Same Flynn.Events namespace, so nothing that publishes or subscribes noticed:
    // see Scripts/Interactables/ScanEvents.cs.
