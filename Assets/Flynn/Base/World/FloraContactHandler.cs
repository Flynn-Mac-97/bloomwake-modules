...
namespace Flynn.World
{
    ...
    public class FloraContactHandler : MonoBehaviour
    {
        ...
        private void Awake()
        {
            // Prefer the renderer whose material actually implements the wind
            // shake — flora prefab roots carry a stock-material SpriteRenderer
            // that GetComponentInChildren grabs first, sending the shake into
            // a material without _WindShake (visible on VisualRoot instead).
            foreach (var sr in GetComponentsInChildren<SpriteRenderer>())
            {
                if (sr.sharedMaterial != null && sr.sharedMaterial.HasProperty(WindShakeID))
                {
                    _spriteRenderer = sr;
                    break;
                }
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            ...
            var pc = other.GetComponent<Flynn.Player.ISpeedModifiable>();
            if (pc != null)
