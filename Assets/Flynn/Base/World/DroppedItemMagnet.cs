...
namespace Flynn.World
{
    ...
    public class DroppedItemMagnet : MonoBehaviour
    {
        ...
        private void Collect(ItemDefinition def)
        {
            ...
            // A full pouch REFUSES: the item stays in the world and re-asks in a moment. The old
            // path destroyed unconditionally — full happy flight, absorb sparkle, then the item
            // silently ceased to exist. Worst read in the whole loop.
            var inv = ItemReceiver.Current;
            if (inv == null) return;
            int added = inv.TryAddItem(def, _item.Count);
            if (added >= _item.Count)
            {
                Destroy(gameObject);
                return;
            }
            if (added > 0) _item.Configure(def, _item.Count - added);
            _delay += 1.5f;   // sit out a beat, then quietly try again (space may have freed up)
        }
    }
