...
using Flynn.Player;
// Flynn.Transmitter / Flynn.Tutorial deliberately NOT imported: those are one game's props,
// and the world facts they used to supply now arrive through WorldStateSource instead.
namespace Flynn.Npc
{
    public class DialogueManager : MonoBehaviour
    {
        ...
        private Flynn.Contracts.IDialogueMovementLock _playerLock;
        ...
        // Unify bridge: when a composition-layer view (CozyDialogue's FieldDialogue) drives the
        // visible UI, the brain runs with its own panel hidden and surfaces the turn moments as
        // events. Inert when nothing subscribes + _hideOwnPanel is false (default).
        [SerializeField] private bool _hideOwnPanel;
        public event System.Action<string> SessionOpened;   // npc display name
        public event System.Action SessionClosed;
        public event System.Action ThinkingStarted;
        public event System.Action<string> ReplyProduced;    // final spoken line
        ...
        /// Feed a player turn from an external view (the composition bridge) — same path as
        /// SubmitPlayerInput, but the text comes from the caller instead of this panel's input.
        public void SubmitPlayerText(string input)
        {
            if (_isWaitingForReply) return;
            if (string.IsNullOrWhiteSpace(_activeNpcId)) return;
            input = input?.Trim();
            if (string.IsNullOrEmpty(input)) return;
            if (TryHandleChatCommand(input)) return;

            var sceneLlm = ResolveSceneLlm();
            bool llmEnabled = sceneLlm != null && sceneLlm.HasValidSettings();
            if (llmEnabled) { StartCoroutine(HandleAgentTurn(input, sceneLlm)); return; }
        ...
        private IEnumerator HandleAgentTurn(string playerInput, SceneLlmManager sceneLlm)
        {
            ...
            ReplyProduced?.Invoke(spokenLine);
        ...
        /// Assign to describe the current world to the LLM — one "- fact" per line.
        ///
        /// This USED to query TransmitterStation / SolarCollector / SignalRelay directly, which
        /// welded the whole LLM stack to one game's tutorial props. Inverted: the game supplies
        /// the description, dialogue just asks for it. Unset simply means "no world facts",
        /// which is what a fresh scene should say anyway.
        public static System.Func<string> WorldStateSource;

        private static string GatherWorldState()
        {
            var src = WorldStateSource;
            if (src == null) return string.Empty;
            return (src() ?? string.Empty).TrimEnd();
        }
        ...
        private void CheckTrustMilestone(int trustBefore, int trustAfter)
        {
            ...
            if (wasLocked.Count > locked.Count)
            {
                ...
                var codexPanel = DialogueHost.Codex;
        ...
        {
            /* Resolved by INTERFACE, not by concrete type. FindFirstObjectByType cannot take an
               interface, so this sweeps MonoBehaviours once and caches the hit — it only ever
               runs on the first line of a conversation. */
            if (_playerLock == null)
            {
                var all = FindObjectsByType<MonoBehaviour>(FindObjectsSortMode.None);
                for (int i = 0; i < all.Length; i++)
                    if (all[i] is Flynn.Contracts.IDialogueMovementLock l) { _playerLock = l; break; }
            }
            if (_playerLock != null)
            {
                _playerLock.IsMovementLocked = locked;
                if (locked && faceTarget != default)
                    _playerLock.FacePoint(faceTarget);
            }
        }
        ...
        private void RefreshItemsRow()
        {
            ...
            var inv = DialogueHost.Inventory;
            if (inv == null)
            {
                _itemsRow.style.display = DisplayStyle.None;
                return;
            ...
            {
                if (!inv.TryGetSlot(i, out var slotItem, out int slotCount)) continue;

                string name = !string.IsNullOrWhiteSpace(slotItem.displayName) ? slotItem.displayName : slotItem.name;
                string countSuffix = slotCount > 1 ? $" ×{slotCount}" : "";
