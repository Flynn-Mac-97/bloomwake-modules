Shader "Flynn/Sprite"
{
    Properties
    {
        ...
        // Stencil, declared as ENUMS on purpose. These are CompareFunction / StencilOp values, not
        // numbers: as plain Floats the inspector offers a free text field, and one stray value here
        // is invisible-sprite territory. Flynn/IslandTopFill writes stencil bit 1 across the whole
        // island silhouette (VibeLayers clips its shadows to it), so any world sprite that TESTS
        // stencil gets discarded wherever it overlaps the ground — which reads exactly like a
        // sorting bug and cannot be fixed by any sorting layer. Leave Comp on Always unless you
        // genuinely want a sprite masked by the island.
